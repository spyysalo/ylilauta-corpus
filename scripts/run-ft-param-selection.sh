...
TESTSET="$DATADIR/ylilauta-dev.txt"

declare -a TRAINSETS=(
    "$DATADIR/ylilauta-train-100.txt"
    "$DATADIR/ylilauta-train-316.txt"
    "$DATADIR/ylilauta-train-1000.txt"
    "$DATADIR/ylilauta-train-3162.txt"
    "$DATADIR/ylilauta-train.txt"
)

declare -a PARAMS=(
    ""
    "-minn 3 -maxn 3 -epoch 25"
    "-minn 3 -maxn 5 -epoch 25"
    "-minn 5 -maxn 5 -epoch 25"
    "-minn 3 -maxn 7 -epoch 25"
)

wordvecs=$(find "$WORDVECDIR" -name '*.vec' -o -name '*.bin')
echo "Wordvecs found in $WORDVECDIR: $wordvecs" >&2
